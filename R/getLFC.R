...
#' @param countData A data frame contains the raw counts. Rows must be the sample IDs and samples coming from the same experimental group should layout together (e.g. first 10 rows are label 1 and next 10 rows are label 2). Columns must be the unique gene symbols. Data should be filtered before entering this step but no need of normalization.
...
#' @import DESeq2
#' @import stats
#' @return A named list of the log fold changes with corresponding gene symbols.
#' @export
#'
